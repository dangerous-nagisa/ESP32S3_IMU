...
constexpr float CALIBRATION_MAX_GYRO_NORM = 0.20f;
...
GyroBias calibrate_icm42688_gyro(const Icm42688Context &ctx) {
    ...
    for (int i = 0; i < CALIBRATION_SAMPLES; ++i) {
        ...
        if (ret == ESP_OK && is_sample_still_for_calibration(sample)) {
            bias.x += sample.gx;
            bias.y += sample.gy;
            bias.z += sample.gz;
            ++valid_samples;
    ...
    ESP_LOGI(TAG, "ICM42688 gyro bias[rad/s]=[%+.5f, %+.5f, %+.5f] from %d still samples",
...
extern "C" void app_main(void) {
    ...
    espp::MadgwickFilter arm_filter(MADGWICK_BETA_RUN);
    espp::MadgwickFilter torso_filter(MADGWICK_BETA_RUN);
